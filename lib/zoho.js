...
async function smartFetch(type, forceRefresh) {
  ...
  // Conservative pre-filter: the LIST response (not just detail) already
  // includes approver_id for single-approver items — confirmed by an
  // earlier debug session (see debugListFields below). That means most
  // items belonging to someone else can be identified and skipped BEFORE
  // ever spending a detail call on them, instead of fetching full detail
  // for the entire company-wide queue and only filtering at the very end.
  //
  // This only ever EXCLUDES an item when the list explicitly says its
  // approver is someone other than Jatin — anything ambiguous (no
  // approver_id at this level, e.g. a multi-approver workflow) is kept
  // and still goes through the existing full detail-fetch + approvers_list
  // check, exactly as before. This can only reduce wasted calls, never
  // cause a real pending item to be missed.
  const relevantItems = currentListItems.filter(item =>
    !item.approver_id || item.approver_id === JATIN_USER_ID
  );
  const skippedCount = currentListItems.length - relevantItems.length;
  if (skippedCount > 0) {
    console.log(`${type}: ${currentListItems.length} pending company-wide, ${skippedCount} confirmed not Jatin's (skipped), ${relevantItems.length} to actually check`);
  }

  // Step 2: Find what's new or changed (only among items that could
  // plausibly be Jatin's)
  const toFetch    = [];
...
// completely bypassing the POs delta-cache — meaning every pending bill
// with a linked PO re-fetched that PO's full detail from Zoho on EVERY
// single bills refresh, even when nothing about that PO had changed since
// the last check. This checks the same persisted cache the POs tab already
// maintains first, and only falls back to a real fetch (caching the
// result for next time, for either tab) when it's genuinely not there.
async function getCachedPODetail(poId) {
  ...
  if (detail) {
    STORE.pos.detailCache[poId]  = detail;
    STORE.pos.listSnapshot[poId] = detail.last_modified_time || '';
    await persistStore('pos');
  }
  return detail;
}
