...
async function hydrateFromPersistedStore(type) {
  ...
  try {
    ...
    // checkedSnapshot is the new shape — old persisted caches (pre-redesign)
    // won't have it, so they're treated as "nothing usable yet" and this
    // does one clean bootstrap under the new, much smaller structure.
...
async function getCachedPODetail(poId) {
  ...
  if (linkedPOCache[poId]) return linkedPOCache[poId];
