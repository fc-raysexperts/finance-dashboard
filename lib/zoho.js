...
// ── TOKEN MANAGER ─────────────────────────────────────────────
// This is the ONLY thing changed in this file. Token fetching now goes
// through lib/zohoToken.js, shared with pmos.js and project-financials.js
// — fixes the repeating 401 caused by 3 separate token caches racing each
// other after an idle period (see lib/zohoToken.js for the full
// explanation). Nothing else below this point is touched.
...
async function zohoGET(path, params = {}) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      // 401 retry: re-check the shared token store fresh (attempt 1), then
      // force a genuinely new refresh as a last resort (attempt 2) — this
      // is what makes the shared cache in lib/zohoToken.js actually able
      // to recover instead of just failing once on the first 401.
