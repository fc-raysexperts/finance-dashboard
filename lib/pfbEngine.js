...
// Shared by both PFB-alignment and PO-vs-Bill checks: a single,
// direction-aware comparison. Confirmed real bug this fixes: the old
// logic used Math.abs() on the variance, so a rate well BELOW the
// reference (favorable) got the exact same "reject"/Over Budget label as
// a rate well ABOVE it (unfavorable) — direction was being thrown away.
function compareValue(actual, reference) {
  if (reference == null || reference <= 0 || actual == null) {
    return { variance: null, status: 'na' };
  }
...
// Worse-of-two-statuses, used to roll qty-status and rate-status into one
// overall status per line, instead of basing the whole row on rate alone.
const STATUS_SEVERITY = { ok: 0, na: 0, under_caution: 1, over_caution: 1, under_severe: 2, over_severe: 2 };
  ...
  return (lineItems || []).map(li => {
    ...
    if (!match || confidence < 0.3) {
      return {
        ...
        pfbQty: null, pfbRate: null,
        qtyVariance: null, rateVariance: null,
        qtyStatus: 'na', rateStatus: 'na',
        pfbMatch: null, status: 'na',
        matchTier: 0,
        comment: 'No PFB scope match found — item outside standard budget (acceptable for services, freight, legal etc.)',
      };
    }

    const rateCmp = compareValue(li.rate, match.rate);
    const qtyCmp  = compareValue(li.quantity, match.qty);
    // Per explicit request: Overall reflects whether the line's ACTUAL
    // total (Qty x Rate) came in above or below the PFB-budgeted total for
    // that scope item — a genuinely separate signal from the Qty/Rate
    // statuses above. A line can have Qty High + Rate Low netting to an
    // Overall of OK (correctly reflecting the total came out fine even
    // though the components individually drifted), and that's intentional
    // — Qty/Rate columns still show the individual drift either way.
    const amountCmp = compareValue(li.item_total, match.amount);

    return {
      lineItem: li.name,
      qty: li.quantity, rate: li.rate, amount: li.item_total,
      pfbQty: match.qty, pfbRate: match.rate, pfbAmount: match.amount,
      qtyVariance: qtyCmp.variance, rateVariance: rateCmp.variance, amountVariance: amountCmp.variance,
      qtyStatus: qtyCmp.status, rateStatus: rateCmp.status,
      pfbMatch: `${match.scopeName} (${match.pfbHead})`,
      status: amountCmp.status,
...
// Used everywhere downstream that needs to know "is this severe" or "is
// this a caution" without caring which direction (over/under) it was —
// e.g. for badge colors, severity sorting, and recommendation text.
function isSevere(status)  { return status === 'over_severe'  || status === 'under_severe';  }
function isCaution(status) { return status === 'over_caution' || status === 'under_caution'; }

module.exports = { generatePFB, checkPOAlignment, findBestPFBMatch, nameSimilarity, compareValue, worseStatus, isSevere, isCaution, DEFAULT_RATES, PFB_ITEMS_DEF, SCOPE_TO_RATEKEY };
