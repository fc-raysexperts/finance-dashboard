...
async function runPOCompliance(po, pfbTotal, advancePaidByPO = {}, aiResults = {}) {
  ...
  // Real subtype detection — confirmed via real samples in this project
  // (PO/26-27/00402, SO/26-27/00112, LO.26-27.00095): the type is the
  // first 2 letters of the PO number itself, before the first "/" or
  // ".". No separate structured field distinguishes this — the number
  // prefix IS the source of truth. Verified against the real
  // applicability checklist (Google Sheet) provided for this project.
  const poNumberStr = String(po.purchaseorder_number || '');
  const poSubtype = /^LO/i.test(poNumberStr) ? 'LO' : /^SO/i.test(poNumberStr) ? 'SO' : 'PO';
  const isLO = poSubtype === 'LO';
  const isSO = poSubtype === 'SO';
  ...
  // 3. Delivery Details — real, confirmed: PO only (SO/LO don't have a
  // physical delivery-scheduling concept the same way).
  if (poSubtype === 'PO') {
    checks.push({
  ...
  // 4. Shipment Terms — real, confirmed: PO only.
  if (poSubtype === 'PO') {
  ...
  if (hasAdvance && !isLO) {
    ...
    // Confirmed NOT applicable to Land Orders (no vendor "dispatch"
    // concept for land) — real applicability sheet + explicit confirmation.
    checks.push({
  ...
  // 13. LD Clause — not applicable to Land Orders (confirmed).
  ...
  // Not applicable to Land Orders (confirmed), kept as its OWN separate
  // check from LD Clause above, per explicit request not to combine them.
  if (hasLD && !isLO) {
  ...
  // items are actually present, AND only for PO (not SO/LO — confirmed
  // via the real applicability sheet: no serial-numbered equipment
  // concept for services or land).
  if (warrantyItems.length > 0 && poSubtype === 'PO') {
  ...
  // 22. Logistics Coordination — PO only per the real applicability sheet
  // (no physical logistics/dispatch coordination concept for SO/LO).
  if (poSubtype === 'PO') {
  ...
  // Confirmed against the real reference checklist: TDS isn't a required
  // check for POs (any variant — PO, SO, LO). TDS genuinely belongs on
  // Bills (kept there, unchanged) — a PO is a commitment to buy, not a
  // payment event, so there's nothing to deduct TDS from yet at this
  // stage. Removed here.
  ...
  // 18. PR–PO Matching
  // 26. PR–PO Matching — PO only per the real applicability sheet
  // (SO/LO don't necessarily go through the same PR process).
  if (poSubtype === 'PO') {
  ...
  const AI_CHECK_IDS = ["advance_clarification","ld_clause","ld_consistency","warranty","serial_mapping","logistics","notes_tc","pr_match","approval_status","delivery_confirmation"];
  for (const check of checks) {
    ...
    if (ai) {
      ...
      check.passed = docs.length === 0 ? 'no-evidence' : null; // distinct: 'no-evidence' will never resolve just by waiting
      check.value = docs.length === 0 ? 'No attachment to verify' : 'Pending AI review';
      check.comment = docs.length === 0
        ? 'No attachment is present on this PO for AI to review — this check cannot be verified without supporting documentation.'
        : 'This check requires AI review of attachment content, which hasn\'t run yet for this PO — check again shortly, or after the next scheduled compliance check.';
...
async function runBillCompliance(bill, linkedPO, pfbTotal, aiResults = {}) {
  ...
  // Real subtype detection — now shared with aiComplianceEngine.js via
  // lib/billSubtype.js, so the two can never silently drift apart again
  // (which is exactly what happened before: Bill Type was being read
  // from the wrong location in two separate places). 5 real subtypes:
  // Service, Supply, Supply-FA, O&M, Expense.
  const realBillType = getRealBillType(bill);
  const billSubtype = classifyBillSubtype(realBillType);
  const billTypeNorm = realBillType.toLowerCase().replace(/\s+/g, '');
  ...
  const isService = billSubtype
    ? billSubtype === 'Service' || billSubtype === 'O&M'
    : hasProductType
  ...
  // When Bill Type is genuinely blank in Zoho (real, common case — it's
  // an optional field employees don't always fill in), the SAME single
  // Gemini call that evaluates the AI-dependent checks below is also
  // asked to classify the bill's real subtype from its full context and
  // attachments (see buildBillPrompt) — one call, not two. That
  // classification, once available, is preferred here over the old
  // crude isService-based guess, since it's a genuine judgment based on
  // real content rather than a keyword heuristic.
  const subtypeKnown = !!billSubtype;
  const effectiveSubtype = billSubtype || aiResults.classified_subtype || (isService ? 'Service' : 'Supply');
  const needsPOReference   = effectiveSubtype !== 'Expense';
  const needsNatureOfSupply= effectiveSubtype !== 'Expense';
  const needsLD            = effectiveSubtype === 'Service' || effectiveSubtype === 'Supply' || effectiveSubtype === 'Supply-FA';
  const needsCompletionProof = effectiveSubtype !== 'Expense'; // Service/Supply/Supply-FA/O&M — corrected per explicit confirmation
  const needsClientAcceptance = effectiveSubtype !== 'Expense'; // same correction
  const needsWarrantyCheck  = effectiveSubtype !== 'Expense'; // Service/Supply/Supply-FA/O&M — corrected: O&M does need this too
  const needsGRN            = effectiveSubtype === 'Supply' || effectiveSubtype === 'Supply-FA';
  const needsRetention      = effectiveSubtype !== 'Expense';
  // RCM and Ex-Works are deliberately NOT hard-gated by subtype here —
  // per explicit confirmation, both are fundamentally item-driven (does
  // this bill actually have a taxi/rent/freight-type line item?), so a
  // Service bill could still genuinely need RCM or Ex-Works if it has
  // the right kind of item on it. These subtype expectations are instead
  // conveyed to Gemini as a SOFT signal in the AI prompt (see
  // buildBillPrompt's "rcm"/"ex_works" instructions) — real applicability
  // per the sheet: RCM/Ex-Works most likely for Supply/Supply-FA/O&M,
  // least likely for Expense (RCM) or Service+Expense (Ex-Works), but
  // never an absolute code-level block.
  const subtypeAllowsRCM    = effectiveSubtype !== 'Expense'; // soft signal only, conveyed via prompt
  const needsExWorks        = effectiveSubtype === 'Supply' || effectiveSubtype === 'Supply-FA' || effectiveSubtype === 'O&M'; // soft signal only, conveyed via prompt
  const needsFreight        = true; // all subtypes per the sheet, including Expense — freight can appear standalone anywhere
  const isExpenseUnder3L    = effectiveSubtype === 'Expense' && totalAmt < 300000;
  ...
  // 4. PO Reference — not applicable for Expense bills (confirmed:
  // these frequently and legitimately have no PO at all). Not shown at
  // all when not applicable — only genuinely applicable checks should
  // ever appear in the CC Table or Review Report.
  ...
  // 6. Nature of Supply — not applicable for Expense bills (confirmed).
  if (needsNatureOfSupply) {
    checks.push({
  ...
  // 14. LD Clause — now AI-judged. Not applicable for O&M/Expense per
  // the real applicability sheet. Not shown at all when not applicable.
  if (needsLD) {
    checks.push({
  ...
  // 15. Milestone / Completion Proof — now AI-judged. Applies to
  // Service, Supply, Supply-FA, and O&M — NOT Expense (confirmed).
  ...
  // 16. GRN Attachment — now AI-judged. Applies only to Supply/Supply-FA
  // (physical goods received) per the real applicability sheet — NOT
  // Service/O&M/Expense.
  if (needsGRN) {
    checks.push({
      ...
      passed: null,
      value: 'Pending AI review',
      comment: 'Requires AI review of attachment content to confirm a genuine GRN — hasn\'t run yet for this bill.',
    });
  }

  // 19. Client Acceptance — now AI-judged. Applies to Service, Supply,
  // Supply-FA, and O&M — NOT Expense (confirmed correction). Note: the
  // NATURE of acceptable evidence naturally differs by subtype (a
  // technical sign-off for Service/O&M vs. a goods/QC acceptance for
  // Supply/Supply-FA) — the AI prompt conveys this as guidance, kept as
  // ONE check here rather than hard-coding separate logic paths.
  if (needsClientAcceptance) {
    checks.push({
      id:'client_acceptance', name:'Client Acceptance',
      passed: null,
      value: 'Pending AI review',
      comment: 'Requires AI review of attachment content to confirm genuine acceptance/sign-off appropriate to this bill\'s nature — hasn\'t run yet for this bill.',
    });
  }

  // 20. Bill Type Accuracy — real bug fixed here too: was reading
  // bill.bill_type directly (usually blank, per the custom-field
  // discovery above) — now uses realBillType, extracted the same
  // correct way.
  const billTypeMatches = !realBillType || (isService ? billTypeNorm.includes('service') || billTypeNorm.includes('o&m') : !billTypeNorm.includes('service'));
  checks.push({
    id:'bill_type_accuracy', name:'Bill Type Accuracy',
    passed: billTypeMatches,
    value: realBillType || (aiResults.classified_subtype ? `Not set — AI classified as ${aiResults.classified_subtype}` : 'Not set'),
    comment: !realBillType
      ? aiResults.classified_subtype
        ? `Bill Type not set in Zoho — AI classified this bill as "${aiResults.classified_subtype}" based on content/attachments; verify this classification looks correct`
        : `Bill Type not set on this bill — verify correct classification (Service vs Goods/Supply)`
      : billTypeMatches
        ? `Bill Type "${realBillType}" matches item-level classification (${isService?'Service':'Goods'})`
        : `Bill Type "${realBillType}" does NOT match item-level classification (items look like ${isService?'Service':'Goods'}) — possible supply/service mismatch`,
  });
  ...
  // Not shown at all for Expense bills (subtype-excluded).
  const warrantyItems = lineItems.filter(l => needsWarranty(l.name));
  if (needsWarrantyCheck) {
    checks.push({
  ...
  // NEW — Expense bills under ₹3 lakh specifically require RP Sir's
  // actual signature on the attachment, with the signed-off amount
  // matching the invoice amount — confirmed via the real applicability
  // sheet's dedicated note for this exact scenario. Distinct from
  // bill_no_po above: this applies regardless of whether a PO is linked,
  // specifically for low-value Expense bills.
  if (isExpenseUnder3L) {
    checks.push({
      id:'expense_rp_sir_signoff', name:'Expense Bill Sign-Off (RP Sir)',
      passed: null,
      value: 'Pending AI review',
      comment: 'Requires AI review to confirm RP Sir\'s genuine signature is present on the attachment, and that the signed-off amount matches this bill\'s invoice amount — hasn\'t run yet.',
    });
  }

  // 21. Retention (10% on service-type bills) — now AI-judged (was: notes
  // containing "retention"/"10%"/"hold" — boilerplate problem, confirmed
  // on a real Terms & Conditions sample where this exact language is
  // standard template text present on nearly every service bill). Not
  // shown at all for Expense bills.
  if (needsRetention) {
    checks.push({
  ...
  const AI_CHECK_IDS_BILL = ['ld_clause','completion_proof','grn','client_acceptance','warranty','bill_no_po','retention','rcm','freight_bills','ex_works','expense_rp_sir_signoff'];
  for (const check of checks) {
    if (AI_CHECK_IDS_BILL.includes(check.id)) {
      ...
      if (ai) {
        ...
        check.aiVerified = false;
        // Real bug fixed: previously showed "hasn't run yet — check
        // again shortly" even when this bill has ZERO attachments at
        // all — genuinely misleading, since no amount of waiting would
        // ever change that outcome. Distinguish the two honestly, using
        // a distinct 'no-evidence' state so the UI can show a different
        // icon than the "still pending" hourglass.
        if (docs.length === 0) {
          check.passed = 'no-evidence';
          check.value = 'No attachment to verify';
          check.comment = 'No attachment is present on this bill for AI to review — this check cannot be verified without supporting documentation.';
        }
      }
    }
...
async function runPMOCompliance(pmo, otherPMOSignatures = [], aiResults = {}) {
  ...
  // confirmed values: Project, O&M, NPD, Admin. The check below is
  // deliberately Category-agnostic (never enumerates specific category
  // names) — it only ever looks at the Sub-Category text, so it already
  // works correctly for any Category, including ones not listed here.
  // pmo.payment_type here is the combined "Category / Sub-Category /
  // Type" string built in pages/api/pmos.js, so a simple substring check
  // correctly captures the real sub-category.
  const isPurchaseCategory = payType.toLowerCase().includes('purchase');
  checks.push({
  ...
  // for PO's Warranty and Bill's Bill-Without-PO checks). Also now gated
  // on the real Payment Sub-Category (confirmed via applicability sheet:
  // only applies when Sub-Category is "Purchase", not "Service" —
  // regardless of advance status). Not shown at all when not applicable
  // — only genuinely applicable checks should ever appear in the CC
  // Table or Review Report.
  const materialStatusApplies = isAdvance && isPurchaseCategory;
  if (materialStatusApplies) {
    checks.push({
      id:'material_status', name:'Material Status / Dispatch Confirmation',
      passed: null,
      value: 'Pending AI review',
      comment: 'Requires AI review of attachment content to confirm a genuine vendor email/document about material dispatch readiness — hasn\'t run yet for this PMO.',
    });
  }
  ...
  for (const checkId of ['material_status', 'authorization']) {
    ...
    if (aiResults[checkId]) {
      ...
      check.value = ai.passed ? 'Confirmed by AI review' : 'Not confirmed by AI review';
      check.aiVerified = true;
    } else if (check.passed === null && docs.length === 0) {
      // Real bug fixed: previously showed "hasn't run yet" even when
      // this PMO has ZERO attachments at all — genuinely misleading.
      check.passed = 'no-evidence';
      check.value = 'No attachment to verify';
      check.comment = 'No attachment is present on this PMO for AI to review — this check cannot be verified without supporting documentation.';
    }
  }
...
function getComplianceStatus(checks) {
  ...
  //   passed === 'no-evidence' -> analyzed, but there's genuinely no attachment to verify against —
  //                               distinct from 'pending' since this will NEVER resolve just by waiting
  ...
  const known    = checks.filter(c => c.passed !== null && c.passed !== 'no-evidence');
  ...
  // Real fix: po_ref (Bills) unconditionally fails whenever there's no
  // linked PO — but bill_no_po exists specifically to validate the
  // ALTERNATE acceptable path for exactly that situation (management
  // approval + RP Sir's genuine sign-off, confirmed via AI review of the
  // actual attachment). Previously, po_ref's bare "no PO" failure was
  // still treated as critical even after bill_no_po had genuinely
  // confirmed that sign-off was found — directly contradicting what
  // bill_no_po itself had just validated, and forcing a 'fail' status
  // (and downstream, an outright REJECT recommendation) regardless. Once
  // bill_no_po genuinely passes, "no PO" is the expected, accepted
  // state, not a real gap. Safe for PO/PMO checks too — neither ever
  // has a 'bill_no_po' id in its checks array, so this never triggers
  // for them.
  const billNoPo = checks.find(c => c.id === 'bill_no_po');
  const alternateApprovalConfirmed = billNoPo?.passed === true;
  const isResolvedPoRef = c => c.id === 'po_ref' && alternateApprovalConfirmed;

  const critFail = failed.filter(c => !isResolvedPoRef(c) && critical.includes(c.id));
  if (critFail.length > 0) return 'fail';
  const failedExcludingResolvedPoRef = failed.filter(c => !isResolvedPoRef(c));
  if (failedExcludingResolvedPoRef.length > 0) return 'warn';
  if (uncertain.length > 0) return 'warn';
  if (checks.some(c => c.passed === null || c.passed === 'no-evidence')) return 'pending';
  return 'pass';
}
