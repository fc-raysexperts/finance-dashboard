...
const AI_CHECK_IDS_PO = [
  ...
  'serial_mapping', 'logistics', 'notes_tc', 'pr_match', 'approval_status',
...
- A generic boilerplate clause repeated near-identically on every contract this company issues does NOT count as genuine, case-specific evidence — e.g. "Liquidated damages as per company policy will apply" appearing in standard Terms & Conditions is NOT sufficient evidence that an LD clause was actually, specifically addressed for THIS transaction; look for something substantive and specific to this PO.
...
6. "notes_tc" — Compare the Zoho Notes/Terms shown above against the attachments and the PO's own header data (vendor, amount, dates). Flag anything that looks inconsistent, suspicious, or contradictory (e.g. Notes referencing a different PO number). Do not flag routine boilerplate as a concern.
7. "pr_match" — Is there a genuine Purchase Requisition (PR) reference/number that this PO can be matched against (e.g. "PR-1234", "PR/24-25/001")?
8. "advance_clarification" — If an advance payment is mentioned (in Notes, Terms, or attachments), is there a clear, specific justification for it (not just a vague mention)?
9. "delivery_confirmation" — Is an advance payment involved AND does it specifically amount to a 100% advance (fully paid before dispatch/delivery)? If so, is there a genuine vendor email/document confirming material is ready to dispatch or has been dispatched? If this PO does NOT involve a 100% advance at all, set "passed": true with a comment saying it's not applicable.
10. "approval_status" — Has this order genuinely been approved SPECIFICALLY by Nidhi Gupta (Co-Director), Seema (HoD), or Rahul Gupta (Director), or another person explicitly named/identified as holding equivalent Director/HoD-level authority? A signature or mark from any other employee (site engineer, supervisor, staff) does NOT count, even if it looks official. Quote the specific approving phrase and name the person who said it in your comment — if you cannot identify the approver as one of these specific authorities, set "passed": false.
...
- RP SIR'S APPROVAL/SIGN-OFF IS A SPECIAL CASE (relevant to "bill_no_po" AND "expense_rp_sir_signoff") — a TEXTUAL claim like "approved by RP Sir" anywhere in the bill/notes is NOT sufficient proof by itself. His approval is confirmed specifically via his physical signature on scanned/photographed documents. A reference image is attached separately (labeled RP_SIR_SIGNATURE_REFERENCE.png) showing real samples of his actual signature. His signature ALWAYS appears in GREEN ink specifically (never any other color), and its shape is a short, rough, incomplete mark most closely resembling the Greek letter gamma (γ) — similar to the two simple curved lines a child draws for their first attempt at a bird. IMPORTANT — his mark is often genuinely small relative to the full page and can sit right next to other, unrelated handwriting (dates, totals, other signatures in different ink colors, dense scribbles) on a small, tilted, or cluttered scan — look carefully at the ENTIRE document, including near every labeled signature line (not just one you assume is his), rather than concluding it's absent after a quick pass. Only mark his sign-off as confirmed if you can visually identify a genuinely matching green-ink, gamma-like signature somewhere in the attachments. A written claim alone, with no matching signature found after a careful look, must be treated as NOT confirmed.
...
async function getAIComplianceForBill(bill, linkedPO, orgKey = 'rays') {
  ...
  if (needsAI) {
    ...
    if (attachments.length > 0) {
      // Real bug found and fixed here: the reference image was only ever
      // attached when there's no linked PO — correct for bill_no_po, but
      // expense_rp_sir_signoff genuinely applies REGARDLESS of whether a
      // PO is linked (per that check's own comment in checklistEngine.js
      // — it's explicitly for low-value Expense bills, independent of
      // PO-linkage). Any Expense-under-₹3L bill that happened to have a
      // linked PO was previously never given the reference image at all
      // — meaning Gemini was asked to confirm a signature match with
      // nothing to actually compare against, guaranteeing a false "not
      // found" every time, regardless of what the real attachment shows.
      const realBillType = getRealBillType(bill);
      const billSubtype = classifyBillSubtype(realBillType);
      const isExpenseUnder3L = billSubtype === 'Expense' && (bill.total || 0) < 300000;
      if (!linkedPO || isExpenseUnder3L) {
