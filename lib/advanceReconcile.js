...
const { getOrgId, getPMOModuleName } = require('./subsidiaries');
...
// Real performance fix: this function always did at least one real Zoho
// network call to check for anything new — even when the very last scan,
// moments earlier, already confirmed nothing had changed. Since POs
// (Advance Reconciliation) and PMOs (Duplicate Detection) are separate
// requests that both depend on this same snapshot, a normal page load
// paid this real network round-trip TWICE, back to back — confirmed
// directly in real terminal output ("scanned 200 PMO records" appearing
// twice within the same few seconds). A short-lived freshness window
// (20s — far faster than any human would notice a genuinely new PMO)
// lets the second of the two calls simply reuse the first one's
// just-completed work instead of redundantly re-scanning.
const SCAN_FRESHNESS_MS = 20 * 1000;

async function updatePMOSnapshot({ timeBudgetMs = 25000 } = {}, orgKey = 'rays') {
  ...
  // form, Energy/OM use the singular form. This was the direct cause of
  // "getAdvancePaidByPO[om] failed... 404" seen in real terminal output.
  const PMO_MODULE = getPMOModuleName(orgKey);
  const snapshotKey = orgScopedKey(KEYS.PMO_FULL_SNAPSHOT, orgKey);
  const snapshot = (await storeGet(snapshotKey)) || { checked: {}, signatures: {}, lastScanTime: 0 };

  if (snapshot.lastScanTime && (Date.now() - snapshot.lastScanTime) < SCAN_FRESHNESS_MS) {
    console.log(`[AdvanceReconcile][${orgKey}] skipping re-scan — already scanned ${Math.round((Date.now() - snapshot.lastScanTime)/1000)}s ago, well within the ${SCAN_FRESHNESS_MS/1000}s freshness window`);
    return snapshot;
  }

  // Claim this scan immediately — writing the updated timestamp right
  // away, before the slow scan work below even starts, rather than only
  // at the very end. This doesn't make the check a perfect distributed
  // lock (a genuinely simultaneous read from another request could still
  // slip through in the brief window before this write lands — confirmed
  // happening occasionally in real testing), but it shrinks that race
  // window from "however long the whole scan+detail-fetch takes"
  // (several seconds) down to a single fast read-then-write, which is
  // the same real, meaningful improvement already proven for the shared
  // Gemini rate limiter.
  snapshot.lastScanTime = Date.now();
  await storeSet(snapshotKey, snapshot).catch(() => {});
  ...
  console.log(`[AdvanceReconcile][${orgKey}] scanned ${scanned} PMO records (stopped: ${stoppedReason || 'end of list'}), ${toFetch.length} within relevance window need detail-fetch`);
  ...
  for (let i = 0; i < toFetch.length; i += 10) {
    if (Date.now() - startTime > timeBudgetMs) {
      timeBudgetHit = true;
      console.warn(`[AdvanceReconcile][${orgKey}] time budget (${timeBudgetMs}ms) reached after ${fetchedCount}/${toFetch.length} — will continue on next call`);
      break;
    }
    ...
    await Promise.all(batch.map(async ({ id, modified }) => {
      try {
        ...
        console.error(`[AdvanceReconcile][${orgKey}] detail fetch failed for PMO ${id}:`, e.message);
      }
    }));
  ...
  snapshot.lastScanTime = Date.now();
  await storeSet(snapshotKey, snapshot);
  if (timeBudgetHit) {
    console.log(`[AdvanceReconcile][${orgKey}] partial pass complete — ${toFetch.length - fetchedCount} PMOs still pending, will resume on next call`);
  }
  return snapshot;
}
...
async function getAdvancePaidByPO(orgKey = 'rays') {
  try {
    const snapshot = await updatePMOSnapshot({ timeBudgetMs: 25000 }, orgKey);
    const map = {};
    ...
    console.error(`getAdvancePaidByPO[${orgKey}] failed, returning empty map:`, e.message);
...
async function getRecentPMOSignatures(orgKey = 'rays') {
  try {
    const snapshot = await updatePMOSnapshot({ timeBudgetMs: 25000 }, orgKey);
    ...
    console.error(`getRecentPMOSignatures[${orgKey}] failed, returning empty list:`, e.message);
    return [];
  }
}
